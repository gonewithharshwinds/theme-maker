...
const CodePanel: React.FC<CodePanelProps> = ({ themeEditorState, themeId }) => {
  ...
  const figmaTokensCode = generateFigmaTokensCode(themeEditorState);
      ...
      >
        <div className="bg-muted/50 flex flex-none items-center justify-between border-b px-4 py-2">
          <TabsList className="h-8 bg-transparent p-0">
            ...
            <TabsTrigger value="figma-tokens.json" className="h-7 px-3 text-sm font-medium">
              figma-tokens.json
            </TabsTrigger>
        ...
        <TabsContent value="figma-tokens.json" className="overflow-hidden">
          <ScrollArea className="relative h-full">
            <CodeBlock
              code={figmaTokensCode}
              language="json"
              className="h-full rounded-none border-0"
            />
            <ScrollBar orientation="horizontal" />
