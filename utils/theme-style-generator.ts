...
export const generateFigmaTokensCode = (themeEditorState: ThemeEditorState): string => {
  const styles = themeEditorState.styles;
  const result: any = {
    colors: {}
  };
  ...
  colorKeys.forEach((key) => {
    const lightVal = styles.light[key as keyof typeof styles.light];
    const darkVal = styles.dark[key as keyof typeof styles.dark];

    if (typeof lightVal === "string") {
      result.colors.light[key] = {
        "$type": "color",
        "$value": colorFormatter(lightVal, "hex")
      };
    }
    if (typeof darkVal === "string") {
      result.colors.dark[key] = {
        "$type": "color",
        "$value": colorFormatter(darkVal, "hex")
      };
    }
  });

  // Include border radius shadow letter-spacing as well
  result.radius = {
    "$type": "dimension",
    "$value": styles.light.radius || "0.5rem"
  };
